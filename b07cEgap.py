...
def optimize(E, delta_E, cff, order):
    #print(f"Optimisingm for E = {E} and delta_E = {delta_E}")
    list_oe, beam = set_up(E, delta_E, cff, order)
    #print(Fore.RED, f"TYPE for delta_E: {type(delta_E)}, {E}", Fore.RESET)
    fwhm = trace(list_oe, beam)[0]
    if type(fwhm) == None:
        return 0
    delta_E_prime = fwhm*2
    #print(Fore.RED, f"TYPE for FWHM: {fwhm}, type for condition {np.abs(delta_E_prime - delta_E)/delta_E}", Fore.RESET)
    while np.abs(delta_E_prime - delta_E)/delta_E > 0.30:
        #print(f"Optimisingm for E = {E} and delta_E = {delta_E}, delta_E_prime = {delta_E_prime}")
        delta_E = delta_E_prime
        list_oe, beam = set_up(E, delta_E, cff, order)
        fwhm = trace(list_oe, beam)[0]
        #print(Fore.RED, f"Type of TKT {type(tkt)}, Intensity: {tkt[0]}", Fore.RESET)
        #print(Fore.RED, f"TYPE for FWHM: {type(fwhm)}, type for condition {np.abs(delta_E_prime - delta_E)/delta_E}", Fore.RESET)
        delta_E_prime = fwhm*2
        plt.close()
    return 2*fwhm
